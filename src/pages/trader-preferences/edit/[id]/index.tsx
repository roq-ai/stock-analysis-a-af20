...
import { getTraderPreferenceById, updateTraderPreferenceById } from 'apiSdk/trader-preferences';
...
function TraderPreferenceEditPage() {
  ...
  const handleSubmit = async (values: TraderPreferenceInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateTraderPreferenceById(id, values);
      mutate(updated);
      resetForm();
